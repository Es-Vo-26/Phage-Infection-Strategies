...
BasicPhage_set_params(a = a, p = p, D = D, S = S, y = y, y1 = y1, m = m, K = K,
...
for (k in 1:length(AdsRate)) {
  for(j in 1:length(AdsRate1)){
    BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = 3.5, y1 = 2.5, m = 1, K = 1,
...
TSlength <- 20000
TSanalyzed <- 0.99
log10.NutCon <- seq(log10(0.6), log10(6), length = 1000)
...
for(j in 1:length(NutCon)){
  ...
  BasicPhage_set_params(a = 0, p = p, D = D, S = NutCon[j], y = y, y1 = y1, m = 1, K = 1,
...
TSlength <- 20000
TSanalyzed <- 0.99
log10.NutCon <- seq(log10(0.60), log10(7), length = 300)
NutCon <- 10^log10.NutCon
Amplitude <- seq(0, 0.95, length = 300)
...
for (k in 1:length(NutCon)) {
  for(j in 1:length(Amplitude)){
    ...
    BasicPhage_set_params(a = Amplitude[j], p = p, D = D, S = NutCon[k], y = y, y1 = y1, m = 1, K = 1,
...
for (k in 1:length(NutCon)) {
  for(j in 1:length(Frequency)){
    ...
    BasicPhage_set_params(a = 0.9, p = Frequency[j], D = D, S = NutCon[k], y = y, y1 = y1, m = 1, K = 1,
                          i = i, i1 = i1, c = 3.14e-04, b = 0.02, s = s, d = d)
