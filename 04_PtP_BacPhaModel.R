...
TL <- 168
p <- 2*pi/TL #0.05678  # oscillation period
# 24 hr
# 168
# 720
# 8760
...
dxdt[0] = (a*D*sin(p*t)+D)*(S-XX0) - y*m*XX0/(K+XX0)*XX1 - y1*m*XX0/(K+XX0)*XX2 - y2*m*XX0/(K+XX0)*XX3 - y3*m*XX0/(K+XX0)*XX4 + (1-b)*s*pow((XX1+XX3),2)/(pow((XX1+XX3),2)+2)*XX3 + (1-b)*s*pow((XX2+XX4),2)/(pow((XX2+XX4),2)+2)*XX4;
...
if(XX3 <= pow(10,-7)){
  ...
  dxdt[3] = i*(1+c)*XX1*XX5 + y2*m*XX0/(K+XX0)*XX3 - m*XX3 - s*pow((XX1+XX3),2)/(pow((XX1+XX3),2)+2)*XX3;
}
...
if(XX4 <= pow(10,-7)){
  ...
  dxdt[4] = i1*(1+c)*XX2*XX6 + y3*m*XX0/(K+XX0)*XX4 - m*XX4 - s*pow((XX2+XX4),2)/(pow((XX2+XX4),2)+2)*XX4;
}

dxdt[5] = b*s*pow((XX1+XX3),2)/(pow((XX1+XX3),2)+2)*XX3 - c*i*XX1*XX5 - d*XX5;

dxdt[6] = b*s*pow((XX2+XX4),2)/(pow((XX2+XX4),2)+2)*XX4 - c*i1*XX2*XX6 - d*XX6;
' 
...
for (k in 1:length(AdsRate)) {
  for(j in 1:length(AdsRate1)){
    BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = AdsRate[k], i1 = AdsRate1[j], c = c, b = b, s = s, d = d)
...
for (k in 1:length(NutCon)) {
  for(j in 1:length(Amplitude)){
    ...
    BasicPhage_set_params(a = Amplitude[j], p = p, D = D, S = NutCon[k], y = 3.5, y1 = 2.5, y2 = 3.5, y3 = 2.5, m = 1, K = 1,
...
for (k in 1:length(NutCon)) {
  for(j in 1:length(Frequency)){
    ...
    BasicPhage_set_params(a = 0.9, p = Frequency[j], D = D, S = NutCon[k], y = y, y1 = y1, y2 = y2, y3 = y3, m = 1, K = 1,
                          i = i, i1 = i1, c = 3.14e-04, b = 0.02, s = s, d = d)
