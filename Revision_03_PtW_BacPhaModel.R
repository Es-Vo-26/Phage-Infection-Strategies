...
dxdt[0] = (a*D*sin(p*t)+D)*(S-XX0) - y*m*XX0/(K+XX0)*XX1 - y1*m*XX0/(K+XX0)*XX2 - y2*m*XX0/(K+XX0)*XX3 - y3*m*XX0/(K+XX0)*XX4 + (1-b)*s/(pow((XX1+XX3)*r,2)+H)*XX3 + (1-b)*s/(pow((XX2+XX4)*r,2)+H)*XX4;
...
if(XX3 <= pow(10,-7)){
  ...
  dxdt[3] = i*(1+c)*XX1*XX5 + y2*m*XX0/(K+XX0)*XX3 - m*XX3 - s/(pow((XX1+XX3)*r,2)+H)*XX3;
}
else{
  dxdt[3] = i*(1+c)*XX1*XX5 - m*XX3 - s/(pow((XX1+XX3)*r,2)+H)*XX3;
}
...
if(XX4 <= pow(10,-7)){
  ...
  dxdt[4] = i1*(1+c)*XX2*XX6 + y3*m*XX0/(K+XX0)*XX4 - m*XX4 - s/(pow((XX2+XX4)*r,2)+H)*XX4;
}
else{
  dxdt[4] = i1*(1+c)*XX2*XX6 - m*XX4 - s/(pow((XX2+XX4)*r,2)+H)*XX4;
}

dxdt[5] = b*s/(pow((XX1+XX3)*r,2)+H)*XX3 - c*i*XX1*XX5 - d*XX5;

dxdt[6] = b*s/(pow((XX2+XX4)*r,2)+H)*XX4 - c*i1*XX2*XX6 - d*XX6;
...
BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                      i = i, i1 = i1, c = c, b = b,  s = s, d = d, H = H, r = r, WP = 19.89975)
...
for(j in 1:length(NutCon)){
  for (k in 1:length(SwitchPoint)){
    ...
    BasicPhage_set_params(a = 0, p = p, D = D, S = NutCon[j], y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b, s = s, d = d, H = H, r = r, WP = SwitchPoint[k])
...
ggplot(results, aes(NutrientSupply, SwitchL, fill = SpecNum))+
  ...
  geom_hline(yintercept = c((sqrt(0.33/0.165-1)/0.5), (sqrt(0.33/0.0033-1)/0.5), (sqrt(0.33/0.001-1)/0.5))) +
...
for(j in 1:length(Lysis)){
  BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                        i = i, i1 = i1, c = c, b = b, s = Lysis[j], d = d, H = H, r = r, WP = 19.89975)
...
for(j in 1:length(NutCon)){
  for (k in 1:length(Lysis)){
    xStart <- c(NutCon[j], Bacteria, Bacteria2, Infected, Infected2, Phage, Phage2)  
    BasicPhage_set_params(a = 0, p = p, D = D, S = NutCon[j], y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b, s = Lysis[k], d = d, H = H, r = r, WP = 19.89975)
...
TSlength <- 10000
TSanalyzed <- 0.99
log10.IndHalfSat <- seq(log10(0.1),log10(2), length = 1000)
...
for(j in 1:length(IndHalfSat)){
  BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                        i = i, i1 = i1, c = c, b = b, s = s, d = d, H = IndHalfSat[j], r = r, WP = 19.89975)
...
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(Bac.min, Bac.max)), col = "green2", cex = 0.25, pch = 20)
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(Bac2.min, Bac2.max)), col = "goldenrod1", cex = 0.25, pch = 20)
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(iBac.min, iBac.max)), col = "green4", cex = 0.25, pch = 20)
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(iBac2.min, iBac2.max)), col = "darkgoldenrod3", cex = 0.25, pch = 20)
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(Pha.min, Pha.max)), col = "brown2", cex = 0.25, pch = 20)
points(log10(c(IndHalfSat2, IndHalfSat2)), log10(c(Pha2.min, Pha2.max)), col = "firebrick", cex = 0.25, pch = 20)
...
log10.IndHalfSat <- seq(log10(0.1), log10(2), length = 100)
IndHalfSat <- 10^log10.IndHalfSat
log10.NutCon <- seq(log10(0.6), log10(8), length = 100)
NutCon <- 10^log10.NutCon
...
for(j in 1:length(NutCon)){
  for (k in 1:length(IndHalfSat)){
    xStart <- c(NutCon[j], Bacteria, Bacteria2, Infected, Infected2, Phage, Phage2)  
    BasicPhage_set_params(a = 0.5, p = p, D = D, S = NutCon[j], y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b, s = s, d = d, H = IndHalfSat[k], r = r, WP = 19.89975)
...
ggplot(results, aes(NutrientSupply, SH, fill = SpecNum))+
  geom_raster(interpolate = T) +
  scale_fill_gradientn(name = "", colors = magma(7)) +
  scale_x_continuous(trans = "log10", expand = c(0, 0)) +
  scale_y_continuous(trans = "log10", expand = c(0, 0))
...
TSlength <- 10000
TSanalyzed <- 0.99
log10.Corr <- seq(log10(0.01), log10(1), length = 1000)
Correction <- 10^log10.Corr
...
for(j in 1:length(Correction)){
  BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                        i = i, i1 = i1, c = c, b = b, s = s, d = d, H = H, r = Correction[j], WP = 19.89975)
...
Correction2 <- sort(rep(Correction, 10))

par(mfrow = c(1,1))
plot(log10(rep(Correction2, 12)), 
     log10(c(Bac.min, Bac.max, Bac2.min, Bac2.max, iBac.min, iBac.max, iBac2.min, iBac2.max, Pha.min, Pha.max, Pha2.min, Pha2.max)),
     type = "n",
     ylim = c(-10, 2),
     xlab = "log10(Correction (s))",
     ylab = "normalized Biovolume")

points(log10(c(Correction2, Correction2)), log10(c(Bac.min, Bac.max)), col = "green2", cex = 0.25, pch = 20)
points(log10(c(Correction2, Correction2)), log10(c(Bac2.min, Bac2.max)), col = "goldenrod1", cex = 0.25, pch = 20)
points(log10(c(Correction2, Correction2)), log10(c(iBac.min, iBac.max)), col = "green4", cex = 0.25, pch = 20)
points(log10(c(Correction2, Correction2)), log10(c(iBac2.min, iBac2.max)), col = "darkgoldenrod3", cex = 0.25, pch = 20)
points(log10(c(Correction2, Correction2)), log10(c(Pha.min, Pha.max)), col = "brown2", cex = 0.25, pch = 20)
points(log10(c(Correction2, Correction2)), log10(c(Pha2.min, Pha2.max)), col = "firebrick", cex = 0.25, pch = 20)
...
##### 'Resource supply and Correction parameter' #####
TSlength <- 10000
TSanalyzed <- 0.99
log10.Corr <- seq(log10(0.01), log10(1), length = 100)
Corr <- 10^log10.Corr
log10.NutCon <- seq(log10(0.6), log10(8), length = 100)
NutCon <- 10^log10.NutCon
...
for(j in 1:length(NutCon)){
  for (k in 1:length(Corr)){
    xStart <- c(NutCon[j], Bacteria, Bacteria2, Infected, Infected2, Phage, Phage2)  
    BasicPhage_set_params(a = 0.5, p = p, D = D, S = NutCon[j], y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b, s = s, d = d, H = H, r = Corr[k], WP = 19.89975)
...
ggplot(results, aes(NutrientSupply, Correction, fill = SpecNum))+
  geom_raster(interpolate = T) +
  scale_fill_gradientn(name = "", colors = magma(7)) +
  scale_x_continuous(trans = "log10", expand = c(0, 0)) +
  scale_y_continuous(trans = "log10", expand = c(0, 0))
...
SW <- 10^log10.SW
log10.Corr <- seq(log10(0.01), log10(1), length = 50)
Corr <- 10^log10.Corr

results <- data.frame(Correction = c(),
                      Switch = c(),
                      concNut = c(),
                      concBac = c(),
                      concBac2 = c(),
...
for(j in 1:length(Corr)){
  for (k in 1:length(SW)){
    BasicPhage_set_params(a = 0, p = p, D = D, S = S, y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b, s = s, d = d, H = H, r = Corr[j], WP = SW[k])
    ...
    results.loop <- data.frame(Correction = Corr[j],
                               Switch = SW[k],
                               concNut = mean(Sim.01.Sub$X1),
                               concBac = mean(Sim.01.Sub$X2),
                               concBac2 = mean(Sim.01.Sub$X3),
...
ggplot(results, aes(Correction, Switch, fill = SpecNum))+
  geom_raster(interpolate = T) +
  scale_fill_gradientn(name = "", colors = magma(7)) +
  scale_x_continuous(trans = "log10", expand = c(0, 0)) +
  scale_y_continuous(trans = "log10", expand = c(0, 0))
...
TSlength <- 10000
TSanalyzed <- 0.99
log10.NutCon <- seq(log10(0.6), log10(30), length = 500) #50 for long
NutCon <- 10^log10.NutCon
...
abline(v = c(-0.25, 0, 0.25, 0.50, 0.75), h = log10(6))
...
log10.NutCon <- seq(log10(0.60), log10(8), length = 400)
NutCon <- 10^log10.NutCon
log10.Period <- seq(log10(2*pi/(24)), log10(2*pi/(8760)), length = 400)
Per <- 10^log10.Period
#8760
...
for (k in 1:length(NutCon)) {
  for(j in 1:length(Per)){
    ...
    BasicPhage_set_params(a = 0.9, p = Per[j], D = D, S = NutCon[k], y = y, y1 = y1, y2 = y2, y3 = y3, m = m, K = K,
                          i = i, i1 = i1, c = c, b = b,  s = s, d = d, H = H, r = r, WP = 19.89975)
